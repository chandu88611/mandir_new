        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <h2 className="text-2xl font-bold mb-2">
            ...
            <p className="text-red-600 flex gap-2 items-center mb-3">
              ...
              <img
                src="/images/Verified-Tag.png"
                alt=""
                className="h-6"
              />
              <img
                src="/images/Donor-Safety-Tag.png"
                alt=""
                className="h-6"
              />
            </p>
            ...
            <div className="  bg-white z-10 my-6 w-full grid grid-cols-3 sticky-donation-card ">
              <button
                className={`py-2 px-4 rounded ${activeSection === 'seva' ? 'border-b-4 border-green-500 font-bold' : ''}`}
                onClick={() => scrollToSection('seva')}
              >
                Products
              </button>
              <button
                className={`mx-2 py-2 px-4 rounded ${activeSection === 'story' ? 'border-b-4 border-green-500 font-bold' : ''}`}
                onClick={() => scrollToSection('story')}
              >
                Story
              </button>
              <button
                className={`mx-2 py-2 px-4 rounded ${activeSection === 'videos' ? 'border-b-4 border-green-500 font-bold' : ''}`}
              ...
              <div className="mt-4">
                <div className="flex justify-between items-center">
                  <span className="text-orange-500 font-bold">
                    {Math.round((Math.round(campaign?.data?.campaign?.donated_amount) / Math.round(campaign?.data?.campaign?.target_amount)) * 100)}%
                    raised
                  </span>
                ...
                <div className="h-2 w-full bg-gray-200 rounded-full my-2">
                  <div
                    ...
                    style={{
                      width: Math.round((Math.round(campaign?.data?.campaign?.donated_amount) / Math.round(campaign?.data?.campaign?.target_amount)) * 100) + "%",
                    }}
                  ></div>
                </div>
                <span className="text-gray-700">
                  ₹{Math.round(campaign?.data?.campaign?.donated_amount)} raised out of ₹{Math.round(campaign?.data?.campaign?.target_amount)}
                </span>
              </div>
              <hr className="my-3" />
              <SocialButtons />
